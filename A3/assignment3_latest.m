...
% Step b: Apply a Gaussian filter to derive smoothed image
sigma = 0.5; % Standard deviation for Gaussian filter
hsize = 2 * ceil(3 * sigma) + 1; % Size of the filter
G = fspecial('gaussian', hsize, sigma);

% Apply the Gaussian filter to smooth the image
smoothed_img = imfilter(gray_img, G, 'replicate');

% Derive gradient images using Sobel operators after smoothing
Gx = fspecial('sobel'); % Sobel filter for x direction
Gy = Gx'; % Sobel filter for y direction

% Apply Sobel filters to get gradients
gradient_x = imfilter(smoothed_img, Gx, 'replicate');
gradient_y = imfilter(smoothed_img, Gy, 'replicate');
...
num_peaks = 12; % Number of peaks to identify
