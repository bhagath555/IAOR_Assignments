...
img_blur = imgaussfilt(img_contrast, 3);  % sigma=2 for moderate smoothing
...
cleaned_img = bwareaopen(dilated_edges, 1000);  % Remove noise smaller than 100 pixels
cleaned_img = bwmorph(cleaned_img, 'fill');
se = strel('square', 2);  % Structuring element for dilation
cleaned_img = imdilate(cleaned_img, se);
se = strel('disk', 6);
...
subplot(2, 3, 5), imshow(cleaned_img), title('Open count');
subplot(2, 3, 6), imshow(cleaned_img2), title('cleaned2');
...
cc = bwconncomp(cleaned_img2, 8);

pp = regionprops(cc, 'all');